...
export default function PlatformCard({ platformKey, title, pillClass, icon, content, onSaveSuccess }) {
  ...
  const handleSaveCorrection = async () => {
    try {
      ...
      await saveCorrection(platformKey, originalStr, editableContent, learnedRuleInput || `Creator corrected ${title} for voice match`);
      ...
      {isEditing && (
        <div style={{
          ...
          <input
            ...
            style={{
              width: '100%', backgroundColor: 'var(--theme-surface)',
              border: '1px solid var(--theme-border)', borderRadius: '4px',
              padding: '8px 12px', color: 'var(--theme-text-main)',
